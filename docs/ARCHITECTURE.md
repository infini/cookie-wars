...
모든 난이도는 `enemyWaveId`로 같은 `giant-boss-duel`을 참조하고 `enemyCount`를 1로 둡니다. 난이도와 스테이지 selector는 개체 수를 바꾸지 않고 보스 HP·공격력·이동 속도와 원반 레벨만 높입니다. 보스 원본 능력과 이미지는 `monsters.json`의 `cookie-tyrant` 행에서 읽습니다.
...
거대 원반은 일반 원반과 별개의 저장형 소모 무기입니다. `BattleScreen`이 보유량과 살아 있는 보스를 확인한 뒤 `GameContext.useGiantDisc`로 1개를 소모하고 엔진의 `throwGiantDisc`를 호출합니다. 엔진은 `giant-disc.json`에서 피해 30배, 속도 배율, 공격 반경을 읽어 `source: giant` 발사체를 만들고 화면은 같은 테이블의 너비 비율과 색상으로 화면 약 1/3 크기의 회전 원반·두 겹 맥동 오라를 그립니다.

`BattleScreen`은 도로와 진입로가 없는 넓은 잔디 중세 쿠키 왕국 안뜰을 전체 배경으로 사용합니다. HUD에는 난이도·전투 번호·남은 보스만 표시합니다. 보스 최대 렌더 크기는 `battle-ui.json`의 112이고 성과 봇은 아래쪽 좁은 슬롯에 배치합니다. 아군 원반은 파랑·금색, 보스 원반은 빨강·검붉은색입니다.
...
- `__tests__/config.test.ts`: 15개 난이도, 같은 난이도의 단계 성장, 단일 보스 참조 무결성, 거대 원반 30배·화면 1/3 규칙, 5종 원반·5종 봇의 능력 격차, 5단계 음량
...
- `__tests__/battleEngine.test.ts`: 모든 난이도의 단일 보스 생성, 정지 보스 명중, 공격 반경 제한, 중첩 발사, 봇 자동 발사, 성의 자동 공격 금지, 성 원반 2배·거대 원반 30배 피해
