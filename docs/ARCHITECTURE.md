...
저장 버전은 5입니다. 이전 버전의 난이도 단위 보상 기록은 해당 난이도의 1번 전투 보상 키로 이전하므로, 2번 이후 전투는 업데이트 후 정상적으로 보상을 받을 수 있습니다. 이전 단일 `discOwned`와 `discLevel`은 첫 번째 원반의 소유 여부와 레벨로 이전합니다. 이전 단일 봇 ID는 `save-migrations.json`의 별칭으로 첫 쿠키봇에 수량을 합산하고, 이전 몬스터 ID도 새 다등급 몬스터 ID로 바꿉니다. 존재하지 않는 몬스터 ID는 제거되어 도감 발견 수가 전체 수를 넘지 않습니다. 기존 `clearedDifficultyIds`는 해당 난이도 1승으로 이전하며, 새 20승 규칙에 맞춰 실제 해금 인덱스를 다시 계산합니다. 존재하지 않거나 잠긴 난이도가 선택되어 있으면 가장 높은 사용 가능 난이도로 안전하게 복구합니다.

## 전투 엔진

`useBattleEngine`은 UI와 독립된 순수 함수 `advanceBattle`로 전투를 진행합니다. 적의 5개 균등 슬롯 연속 출현과 이동, 모든 활성 봇의 원반 자동 발사, 다중 원반 충돌, 적 원반, 쿠키 성 피해, 승패 판정을 한 단계씩 계산합니다. 난이도가 높아질 때뿐 아니라 같은 난이도에서 승리 수가 오를 때도 selector가 `battle-stage-rules.json`을 적용해 적 수, HP·공격력·이동 속도와 적 원반 레벨을 높입니다. 아직 출현 시간이 되지 않은 적은 이동·표적·공격 대상에서 제외되며, 적 회피 로직은 존재하지 않습니다.
...
쿠키봇은 장착 원반을 각 봇의 테이블 공격 간격마다 자동 발사합니다. 봇 원반은 성 중앙이 아닌 각 봇의 편성 좌표에서 시작하고, `botDiscSizeMultiplier`로 성 원반보다 작게 계산합니다. 이전 원반이 비행 중이어도 공격 간격이 끝나면 다음 원반을 즉시 발사하며, 여러 봇 종류는 각각 독립적으로 움직입니다.

쿠키 성은 스스로 공격하지 않습니다. 전투 중 사용자가 성을 눌렀을 때만 다음 두 조건으로 별도 원반을 발사합니다.

사용자가 성을 누른 순간 장착 원반 레벨의 재사용 대기시간까지 지났을 때만 한 발을 추가합니다. 시간 경과나 전투 프레임 함수는 성 원반을 생성하지 않습니다. 이전 쿠키 성 원반이 화면에 남아 있어도 쿨타임이 끝난 뒤 다음 클릭으로 원반을 발사할 수 있습니다.
...
`BattleScreen`은 세로형 중세 쿠키 왕국 맵을 전체 필드 배경으로 사용합니다. 불필요한 화면 밖 헤더와 하단 조작 설명은 없애고, 난이도·전투 번호·남은 적만 필드 안의 작은 HUD로 표시합니다. 성과 봇은 아래쪽 좁은 편성 슬롯에 모이고 개체 렌더 크기는 `battle-ui.json`에서 바꿉니다. 체력 게이지는 같은 테이블의 외곽선과 HSL 범위를 이용해 체력 0%의 빨강에서 100%의 초록까지 연속 보간합니다.
...
- `__tests__/config.test.ts`: 15개 난이도, 같은 난이도의 단계 성장, 웨이브·몬스터 참조 무결성, 10종 원반·10종 봇의 능력 격차, 5단계 음량
- `__tests__/gameReducer.test.ts`: 스테이지별 최초 보상·재도전 차단, 정확한 20승 해금, 저장 이전, 테이블 기반 구매, 100레벨 이후 강화, 강화 가능 우선 정렬
- `__tests__/battleEngine.test.ts`: 적의 균등 연속 출현, 다등급·보스 패턴, 모든 난이도의 원반 명중, 중첩 발사, 봇 자동 발사, 성의 자동 공격 금지, 성 원반 2배 피해
