...
- `BattleScreen`은 전투 수명주기와 레이어 조합을 맡고, 체력 바·피격 연출·발사체·유닛은 `src/components/battle`의 네 파일로 분리되어 있습니다. 표현 변경이 화면 전체와 엔진 판정에 동시에 번지는 범위를 줄였습니다.
- 피격 연출은 현재 마지막 전투 이벤트 하나만 표시하며 620ms 뒤 사라집니다. 다섯 번의 섬광은 각각 여섯 개 파편으로 제한하고, 넓은 전장 충격파는 성 수동 원반과 거대 원반에만 사용합니다. 이 방식은 View 수를 제한하지만 같은 틱의 복합 이벤트를 누락할 수 있으므로 성능 최적화 계약으로 고정하지 않으며, 제한 길이 이벤트 큐 전환을 `CODE_REVIEW.md`의 P2로 관리합니다.
...
이 평가는 현재 단일 보스 범위에 한정됩니다. 다수 적, 상태 효과, 동시 원반을 추가하면 50ms마다 수행되는 배열 `map`·`filter`·`find`와 새 객체 생성이 개체 수에 따라 증가합니다. `advanceBattle`도 이동·충돌·발사·피해·결과 판정을 함께 수행하므로 프로파일링과 부분 최적화가 어려워질 수 있습니다. 다수 개체 기능을 시작하기 전에 순수 단계로 분리하고, 적·발사체 상한을 정한 뒤 아래 동일 기기 측정으로 회귀를 비교합니다.

1.0.8 APK의 설치와 콜드 스타트 확인은 성능 측정이 아닙니다. 아래 1.0.6 수치는 현재 비교 가능한 실기기 기준선이며, 1.0.8의 전투 프레임·메모리 결과는 같은 조건으로 별도 측정하기 전까지 추정하지 않습니다.
...
adb -s <기기ID> install -r artifacts/cookie-wars-v1.0.8.apk
