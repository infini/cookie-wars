...
- 110종 쿠키 도감은 `FlatList` 가상화를 사용해 화면 주변 카드만 마운트합니다. 쿠키 종류를 더 추가해도 모든 투명 WebP를 화면 진입과 동시에 디코드하지 않습니다.
...
- 클릭커 28대는 공용 native-driver 망치 진행값 하나, 200ms 희귀 판정 interval 하나, 1초 단위 리듀서 정산과 공용 사운드 interval 하나를 사용합니다. 개별 로봇마다 타이머·난수·오디오 플레이어·저장 쓰기를 만들지 않으며 일반 판정은 React 희귀 이벤트도 발행하지 않습니다.
...
1.0.28 APK 빌드·설치 확인은 장시간 성능 측정으로 해석하지 않습니다. 아래 1.0.6 수치가 현재 비교 가능한 실기기 전투 기준선이며, 1.0.28의 장시간 프레임·메모리 결과는 같은 조건으로 별도 측정하기 전까지 추정하지 않습니다.
...
adb -s <기기ID> install -r artifacts/cookie-wars-v1.0.28.apk
...
2026-07-19 `versionCode 28`, `versionName 1.0.27` 릴리스 APK를 빌드했습니다. 결과는 139,245,505바이트(132.79MiB)이고 SHA-256은 `6224ea242f16f88086f3cb2bd88300cd26e0f83f0cff5081f5649967c90a19a2`입니다. 패키지 `com.cookiewars.game`, API 24~36, 세로 고정, arm64-v8a·armeabi-v7a·x86·x86_64, APK v2 서명을 확인했으며 Metro는 1,320개 모듈과 394개 에셋을 번들했습니다.

Xiaomi `24069PC21G`에 `adb install -r`로 1.0.26(27) 위에 덮어 설치해 최초 설치 시각 `2026-07-18 11:59:12`와 231.8조 쿠키·고레벨 강화 데이터를 유지했습니다. 최종 갱신 시각은 `2026-07-19 23:06:43`이고 콜드 실행은 `Status: ok`, `LaunchState: COLD`, `TotalTime: 300ms`였습니다. 메인에서 강화를 연속 12회 왕복한 뒤에도 PID와 전면 `MainActivity`가 유지됐고 AndroidRuntime·ReactNativeJS·`ERR_USING_RELEASED_SHARED_OBJECT` 오류가 없었습니다. 이 점검은 메뉴 해제 경쟁 조건과 첫 화면 정산 경로의 회귀 확인이며 장시간 클릭커 FPS·발열 측정은 아닙니다.

### 1.0.28 빌드·설치 확인

2026-07-20 `versionCode 29`, `versionName 1.0.28` 릴리스 APK를 빌드했습니다. 결과는 141,921,518바이트(135.35MiB)이고 SHA-256은 `c3b93322ffee9b8c8bff3ba8c647c8fd8ac7eb58f2e7a3a214e5b92a47e50e94`입니다. 패키지 `com.cookiewars.game`, API 24~36, 세로 고정, arm64-v8a·armeabi-v7a·x86·x86_64, APK v2 서명을 확인했으며 Metro는 1,351개 모듈과 424개 에셋을 번들했습니다.

Xiaomi `24069PC21G`에 `adb install -r`로 덮어써 최초 설치 시각과 기존 저장을 유지했습니다. 콜드 실행은 342ms였고 기존 고레벨 저장이 보유 쿠키 `8경`, 진화 Lv.858로 복원됐습니다. 새 진화 도감의 81~110단계가 표시됐고 메인↔강화를 12회 연속 왕복한 뒤에도 같은 PID와 전면 `MainActivity`가 유지됐으며 AndroidRuntime·ReactNativeJS·Expo Audio 치명 로그가 없었습니다.

### 1.0.26 빌드·설치 확인

2026-07-19 `versionCode 27`, `versionName 1.0.26` 릴리스 APK를 빌드했습니다. 결과는 139,241,661바이트(132.79MiB)이고 SHA-256은 `be653b3e0faf850075dd533e55eb1862c0be969f1a82e7c2158d7684f585fc6a`입니다. 패키지 `com.cookiewars.game`, API 24~36, 세로 고정, arm64-v8a·armeabi-v7a·x86·x86_64, APK v2 서명을 확인했으며 Metro는 1,317개 모듈과 394개 에셋을 번들했습니다.

Xiaomi `24069PC21G`에 `adb install -r`로 1.0.25(26) 위에 덮어 설치해 최초 설치 시각 `2026-07-18 11:59:12`와 앱 데이터를 유지했습니다. 기기 패키지는 1.0.26(27)이고 사운드 포함 최종 갱신 시각은 `2026-07-19 22:46:29`입니다. 최종 콜드 실행은 `Status: ok`, `LaunchState: COLD`, `TotalTime: 432ms`였고 AndroidRuntime·React Native·Fresco·Expo Audio 치명 오류나 ANR은 없었습니다. 1220×2712 메인 화면에서 프로펠러가 달린 무료 플라잉 로봇과 망치를 든 클릭커 로봇이 서로 구분되고, 보유한 클릭커 1대의 망치 동작과 초당 합산 쿠키 증가를 확인했습니다. 자연 발생 조각의 자동 수집과 최대 28대 편성은 사용자 저장을 임의 변경하지 않아 실제 화면 검증 대상으로 남겼습니다. 28대 로봇은 한 개의 공용 native-driver 진행값, 한 개의 1초 합산 생산 타이머와 초당 최대 5회인 공용 사운드 타이머 하나를 사용하므로 개별 로봇 수만큼 JS 타이머나 오디오 플레이어를 늘리지 않습니다.
