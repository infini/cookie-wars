...
- 마그마·전기는 각각 외부 CC0 60·64프레임 원본을 한 개의 animated WebP로 묶어 Android Fresco에서 네이티브 재생합니다. 일반·슈퍼 크리티컬은 제한된 선 View를 native-driver 진행값 하나로 움직입니다. 해당 희귀 보상 때만 종류별 최대 하나씩 마운트되고 같은 종류가 다시 나오면 교체되며 프레임별 React 상태·JS interval을 만들지 않습니다. 서로 다른 종류가 겹칠 때 WebP 디코더는 최대 두 개이고 선형 연출도 종류별 한 세트로 제한합니다.
...
1.0.24 APK 빌드·설치 확인은 장시간 성능 측정으로 해석하지 않습니다. 아래 1.0.6 수치가 현재 비교 가능한 실기기 전투 기준선이며, 1.0.24의 장시간 프레임·메모리 결과는 같은 조건으로 별도 측정하기 전까지 추정하지 않습니다.
...
adb -s <기기ID> install -r artifacts/cookie-wars-v1.0.24.apk
