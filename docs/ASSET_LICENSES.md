...
| `assets/images/enemies/animations/*.webp` 180개 | 난이도별 보스 30종의 걷기·망치 강공격 OpenAI 이미지 생성 결과 | 프로젝트 전용 원본 |
| `assets/images/bots/animations/*.webp` 30개 | 쿠키봇 5종의 달리기·원반 투척 OpenAI 이미지 생성 결과 | 프로젝트 전용 원본 |
| `assets/images/maps/battle-map-*.jpg` 30종 | 난이도별 고유 전장 OpenAI 이미지 생성 결과 | 프로젝트 전용 원본 |
...
| `assets/images/cookie-fragments/*.webp` 3종 | 쿠키전쟁용 OpenAI 이미지 생성 결과(수집 조각 2종·마그마 화산 폭발 1종) | 프로젝트 전용 원본 |
| `assets/images/vfx/magma-fire-plume.webp` | 쿠키전쟁용 OpenAI 이미지 생성 결과 | 프로젝트 전용 원본 |
...
| `assets/images/vfx/cookie-feedback/critical*.webp` | OpenGameArt `Hit Animation 2`, Sinestesia | CC0 1.0 |
| `assets/images/vfx/cookie-feedback/magma.webp` | OpenGameArt `2D Explosion Animations`, Sinestesia | CC0 1.0 |
| `assets/images/vfx/cookie-feedback/super-critical*.webp`, `electric.webp` | OpenGameArt `2DFX Impact spritesheets`, Esteban Díaz / Inguz Media Studio | 원본 다중 라이선스 중 CC0 1.0 선택 |
...
외부 VFX 원본 페이지, 직접 다운로드 주소와 SHA-256은 `assets/licenses/OPENGAMEART_COOKIE_VFX_SOURCE.txt`에 기록했습니다. 기존 `process_external_cookie_vfx.py`는 보관용 화산 프레임과 번개를 변환하고, 신규 `process_external_cookie_feedback_vfx.py`는 16/64프레임 atlas를 Android 네이티브 애니메이션 WebP 6종으로 변환합니다. 슈퍼는 Inguz의 금색·청록 충격을, 전기는 청록 충격과 Calinou 번개를 합성하지만 모든 픽셀 애니메이션 원본은 외부 CC0 에셋입니다. 앱은 실행 중 네트워크를 사용하지 않습니다.
...
| 마그마 쿠키 조각 획득 | Mixkit `Big fire spell burning` | Mixkit Sound Effects Free License |
| 전기 쿠키 조각 획득 | Mixkit `Strong close thunder explosion` | Mixkit Sound Effects Free License |
...
마그마 조각은 Mixkit `Big fire spell burning`의 4초 화염 주문음을 180ms 간격으로 두 겹 사용합니다. 전기 조각은 저급한 지지직 레이어를 제거하고 Mixkit `Strong close thunder explosion`을 최대 음량 계수 1.0으로 220ms 간격 세 번 재생해 화면을 가로지르는 다중 낙뢰와 맞춥니다. 같은 조각 종류가 다시 나오면 이전 같은 종류의 플레이어와 예약 재생만 즉시 정지·되감고, 일반·슈퍼 크리티컬·다른 조각 소리는 계속 재생됩니다. 파일 출처와 체크섬은 `MIXKIT_SOUND_EFFECT_SOURCE.txt`에 기록했습니다.
...
- [Mixkit Fire Sound Effects · Big fire spell burning](https://mixkit.co/free-sound-effects/fire/)
- [Mixkit Thunder Sound Effects · Strong close thunder explosion](https://mixkit.co/free-sound-effects/thunder/)
