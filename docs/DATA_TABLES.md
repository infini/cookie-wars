...
| `cookie-upgrade-rules.json` | 클릭 힘·일반/슈퍼 크리티컬·조각 발견·자동 생산·쿠키 성 체력의 후속 강화 증가량 |
| `mini-game.json` | A·B 순차 클릭 대결의 10초 단위 시간 범위·카운트다운·입력 크기·눌림·복귀 표현 |
| `cookie-critical.json` | 크리티컬 확률 단위·상한과 획득 배수 |
| `cookie-super-critical.json` | 슈퍼 크리티컬 확률 단위·상한과 획득 배수 |
| `cookie-fragments.json` | 마그마·전기 조각의 확률·보상·5초 수명·등장/획득 연출·음향 |
...
| `boss-animation.json` | 30종 보스의 걷기·망치 강공격 프레임과 전환 시간 |
...
| `cookies.json` | 110종 쿠키의 자동 진화 조건, 이미지, 최종 능력 배율 |
| `cookie-expansion.json` | 51~80단계 쿠키의 개수, 첫 요구 레벨, 간격, 단계별 배율 |
| `save-migrations.json` | 현재 저장 스키마 버전과 이전 데이터 ID 호환 규칙 |
| `../navigation/navigation.json` | 4대 메뉴, 소메뉴 소속·순서·아이콘·제목·기본 화면·배지 |

설정 로더는 레벨, 1회 비용, 개수, 기준 HP·공격력, 원반 크기·속도·쿨타임처럼 개별 계산이 안전 정수로 다루는 필드에 소수나 `Number.MAX_SAFE_INTEGER` 초과 값을 허용하지 않습니다. 전체 쿠키 잔액·누적 획득·원반 강화비 원장은 이 한도와 분리된 BigInt이며 저장할 때 10진 문자열로 직렬화합니다. 성장 배율, 난이도 배율, 좌표와 비율은 유한한 소수를 그대로 보존합니다. 불투명도·정규화 화면 비율·분노 체력 비율은 0~1, HSL 채도·명도는 0~100 범위로 제한합니다. 잘못된 값은 앱 시작 시 정확한 JSON 경로를 포함한 오류로 거부됩니다.
...
클릭 힘은 Lv.1에서 100으로 시작하고 명시된 Lv.8의 800까지 매 단계 +100, 이후에도 레벨마다 +100이며 비용은 ×1.25로 증가합니다. 기존 저장은 레벨만 보관하므로 현재 단계에 맞는 새 클릭 힘이 즉시 소급 적용되고, 크리티컬·조각 보상도 이 최종 클릭 힘을 기준으로 계산합니다.
...
- `rareJudgement.intervalMs`: 모든 클릭커 로봇의 생산을 한 묶음으로 희귀 판정하는 주기. 현재 200ms이며 `productionIntervalMs`를 정확히 나누어야 함
- `formation.*`: 28개 슬롯의 반경·크기와 공용 망치 동작
- `flyingFragmentCollector.*`: 무료 수량, 대기 위치, 출동·귀환 시간과 프로펠러 표현

Lv.1은 0대이고 Lv.2부터 한 단계마다 한 대가 추가되어 Lv.29에서 28대가 됩니다. Lv.30부터 수량은 유지하고 초당 클릭과 로봇 클릭 힘을 함께 올립니다. 28개 개별 타이머를 만들지 않고 공용 망치 애니메이션 하나와 합산 생산 틱 하나를 사용합니다. 희귀 판정은 로봇 개별 클릭마다 반복하지 않고 0.2초 동안 모든 로봇이 만든 쿠키를 한 묶음으로 정확히 한 번만 처리합니다. 일반·슈퍼 크리티컬과 마그마·전기 조각은 수동 클릭과 같은 확률·배수 성장식을 쓰되 별도 천장 카운터를 저장합니다. 조각 보상은 무료 플라잉 클릭커가 공용 생산 묶음에 해당 배수를 적용해 즉시 회수합니다. 망치 소리는 로봇별로 중첩하지 않고 3개 보이스를 순환하는 한 리듬으로 합치며, 기본 5회/초보다 빨라져도 `minimumIntervalMs` 200ms를 지켜 초당 5회를 넘지 않습니다. 무료 플라잉 클릭커 1대는 수동 클릭으로 발견된 5초 조각도 기존처럼 자동 수집합니다.
...
- `probabilityScale`: 정수 난수와 확률 값을 대응시키는 전체 단위. 전기 조각의 0.0125%p를 정수로 정확히 표현하기 위해 네 희귀 보상 공통 200,000
- `maximumChanceUnits`: 실제 발동 확률 상한. 현재 100,000으로 정확히 50%
...
`cookie-upgrades.json`의 `levels[].value`와 무한 강화 규칙의 `valueIncreasePerLevel`은 확률 단위로 해석합니다. Lv.1 값 2,000은 1%, 이후 한 단계당 500은 0.25%p입니다. 계산된 확률 값은 50%에서 제한하지만 강화 단계와 배수는 제한하지 않습니다. 따라서 Lv.1은 1%·10배, Lv.2는 1.25%·11배이며 확률 상한에 도달한 뒤에도 레벨마다 배수가 계속 증가합니다. 클릭마다 0~1 난수 한 번을 정수 단위로 변환해 판정하고, 발동하면 최종 클릭 힘에 해당 배수를 곱합니다. 극단 단계의 결과는 공통 안전 정수 포화 정책을 따릅니다.

### `cookie-super-critical.json`

필드는 `cookie-critical.json`과 같고 현재 `upgradeId`는 `cookieSuperCritical`입니다. Lv.1은 값 200으로 정확히 0.1%·100배이며, 이후 모든 레벨은 확률이 50단위(0.025%p), 배수가 10배씩 증가합니다. 확률은 20,000단위인 10%에서 멈추고 배수와 강화 레벨은 계속 성장합니다. 저장에는 확률값이 아니라 강화 레벨이 있으므로 기존 사용자도 현재 레벨을 기준으로 새 공식을 즉시 소급 적용받습니다. 예를 들어 Lv.20은 `0.1% + 19×0.025%p = 0.575%`입니다. 두 크리티컬은 같은 200,000 눈금을 사용하고 최대 확률 합이 100%를 넘으면 설정 검증이 실패합니다. 클릭 판정은 한 난수에서 슈퍼 구간을 먼저, 그 다음 일반 크리티컬 구간을 배치해 중복 지급하지 않습니다.

### `cookie-fragments.json`

- `probabilityScale`: 조각 발견 확률의 정수 눈금. 네 희귀 보상 공통 200,000
...
마그마 획득음은 `Big fire spell burning`을 180ms 간격으로 두 겹, 전기 획득음은 `Strong close thunder explosion`을 3개 플레이어로 220ms 간격 재생합니다. 같은 종류의 특수 보상이 다시 나오면 그 종류의 시각·오디오만 교체하고, 다른 종류는 최대 4종까지 함께 합성됩니다. 전역 사운드 OFF나 화면 전환은 모든 플레이어와 예약 재생을 취소합니다.

### `cookie-pity.json`

- `enabled`: 네 희귀 보상의 연속 실패 천장 활성화. 현재 `true`
- `criticalPriority`: 두 크리티컬이 같은 클릭에서 확정일 때의 우선순위. 현재 슈퍼 → 일반
- `fragmentPriority`: 두 조각이 같은 클릭에서 확정일 때의 우선순위. 현재 전기 → 마그마

화면에 보이는 확률은 각 클릭의 기본 확률입니다. 별도의 실패 횟수를 4종에 각각 두고 `ceil(확률 눈금 ÷ 현재 확률 단위)`를 최대 시도 횟수로 사용합니다. 예를 들어 일반 크리티컬이 10%면 9회 연속 실패 뒤 10번째를 확정합니다. 자연 슈퍼 발동은 일반과 슈퍼 실패 횟수를 모두 초기화하며, 두 조각 천장이 겹치면 희귀한 전기를 먼저 주고 마그마 천장은 다음 클릭까지 유지합니다. 카운터는 저장 스키마 v13의 `cookiePityMisses`에 저장되므로 재실행해도 리셋되지 않습니다. 계산은 클릭당 정수 비교와 덧셈만 수행하는 O(1)입니다.
...
- `enemyDiscSizeIncreasePerDifficulty`, `maximumEnemyDiscSize`: 레벨당 +1, 최대 48
...
`cookies.json` 배열 순서가 진화 순서입니다. 1~30단계는 클래식 초코칩부터 무한 우주, 31~50단계는 혜성 꼬리부터 쿠키왕국 심장입니다. 51~80단계는 수정 용의 알부터 쿠키 우주핵까지이며, 81~110단계는 양자 불꽃 → 암흑물질 왕관 → 중성자별 → 초신성 불사조 → 블랙홀 문 → 펄서 시계 → 은하 황제 인장 → 차원 용왕 → 무한 프랙탈 → 시간의 강 → 현실 베틀 → 우주 예언자 → 공허 군주 왕좌 → 창세 씨앗 → 다중우주 도감 → 영원의 고리 → 인과율 열쇠 → 꿈의 우주 → 태초의 빛 → 오메가 수정 → 하이퍼노바 심장 → 천상 엔진 → 초차원 함선 → 무한 첨탑 → 세계창조 망치 → 별바다 왕관 → 절대영도 핵 → 창조신룡 → 전우주 관문 → 절대 쿠키 특이점 순서입니다.
...
현재 110종 중 기존 50종의 요구 조건은 3, 9, 15, …, 297입니다. 확장 60종은 `cookie-expansion.json`에 따라 375에서 시작해 12레벨 간격으로 1083까지 이어지고, 공통 능력 배율은 직전 쿠키의 ×1.10을 소수 둘째 자리로 반올림해 최종 ×18900.81까지 증가합니다. 진화 기여 강화는 강화 화면에 보이는 쿠키 관련 8종 전체이며 새 게임의 기본 합계는 8입니다. 조건을 만족한 가장 높은 쿠키가 자동 활성화됩니다. 별도 구매나 수동 장착은 없고 능력 배율은 누적하지 않으며 현재 쿠키 행 하나만 적용합니다. `validateCookieExpansion`이 확장 개수·첫 요구 레벨·간격·배율을 앱 시작 시 검증하고 `CookieImage` 레지스트리가 이미지 키를 확인합니다.
...
현재 30종 보스는 각각 6프레임, 합계 180개 투명 WebP를 사용합니다. 이동 중에는 시작 위치부터 누적한 거리로 걷기 프레임을 고르며, `boss-special-attack.json`의 예고 구간과 실제 `special` 공격 시각에만 망치 3프레임으로 전환합니다. 일반 원거리 공격과 근접 공격에는 강한 망치 준비·충돌·복귀를 재생하지 않습니다. 설정 검증은 모든 보스 ID에 애니메이션 세트가 있고 모든 이미지 키가 고유하며 시퀀스가 실제 걷기 프레임 범위를 벗어나지 않는지 확인합니다.
...
훈장 보너스는 `1 + (보유 훈장 × 능력별 퍼센트) / 100` 배율입니다. 강화 능력과 현재 쿠키 진화 배율을 먼저 계산한 최종 값에 적용하며, 훈장 자체는 진화 레벨에 포함하지 않습니다. 자동 생산에도 같은 최종 배율이 적용되므로 실행 중 생산과 재실행 시 오프라인 정산이 동일한 훈장 효과를 사용합니다. 크리티컬·슈퍼 크리티컬·마그마·전기 조각도 훈장 배율이 적용된 최종 클릭 힘을 기준으로 보상을 계산합니다. `정보 > 쿠키·훈장` 화면은 보유량, 세 현재 배율, 계산식, 다음 미완료 전투를 이 테이블과 저장 진행도에서 직접 읽어 표시합니다. 모든 값은 앱 시작 시 1 이상의 안전 정수인지 검증합니다.
...
- `mainMenus[].leafIds`: 대분류에 속한 실제 화면. 강화는 쿠키·원반·쿠키봇 강화, 정보는 쿠키·훈장·난이도·몬스터
