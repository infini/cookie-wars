...
| `cookies.json` | 80종 쿠키의 자동 진화 조건, 이미지, 최종 능력 배율 |
...
`cookies.json` 배열 순서가 진화 순서입니다. 1~30단계는 클래식 초코칩부터 무한 우주, 31~50단계는 혜성 꼬리부터 쿠키왕국 심장입니다. 51~80단계는 수정 용의 알 → 태양 사자 → 달토끼 → 별고래 → 폭풍 페가수스 → 사막 스핑크스 → 심해 삼지창 → 화산 심장 → 얼음 궁전 → 에메랄드 수호수 → 캔디 기계용 → 시간 도서관 → 꿈나비 → 오로라 성 → 천둥 왕관 → 불사조 왕관 → 세계수 왕관 → 운명 별지도 → 창조 나침반 → 공허 등대 → 은하 열쇠 → 천상 용 → 차원 큐브 → 영원 왕좌 → 우주 방주 → 창세 시계 → 무한 성전 → 다중우주 보석 → 초월 왕관 → 쿠키 우주핵 순서입니다.
