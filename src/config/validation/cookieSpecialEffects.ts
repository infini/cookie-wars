...
export function validateCookieSpecialEffects(value: unknown): UnknownRecord {
  ...
  const effects = array(config.effects, `${path}.effects`).map((item, index) => {
    ...
    numberField(effect, 'sourceFrameCount', effectPath, { integer: true, min: 1 });
  ...
  ordered.forEach((effect) => {
    if (effect.offsetXScreenRatio !== 0 || effect.offsetYScreenRatio !== 0) {
      throw new ConfigValidationError(
        `${path}.effects.${effect.id}.offset`,
        '모든 희귀 보상 연출은 화면 중앙에 있어야 합니다.',
      );
    }
  });
